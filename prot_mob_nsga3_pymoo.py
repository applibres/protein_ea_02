...
Simple Genetic Algorithm for Protein Mutation - NSGA-III
"""

from typing import List

from genetic_operators.crossoever_operators import llm_crossover, uniform_crossover
from genetic_operators.nsga3 import SelectionOperatorNSGA3
...
class pymoo_sga_protein:
    ...
    def __init__(self, scenario, algoritm_params, sim_params, fitness_idxs, output, randomseed):
        ...
        # Kept for backwards compatibility with MOEA/D param packs.
        ...
        if self.popsize < 2:
            raise ValueError("popsize must be >= 2 for NSGA-III")
        ...
        # ── NSGA-III operators ───────────────────────────────────────────────
        nsga3 = SelectionOperatorNSGA3(
            self.n_obj,
            n_partitions=self.n_partitions,
            pop_size=self.popsize,
        )
        self.nsga3          = nsga3                    # full object kept for crossover
        self.selection      = nsga3.selection          # survival update
        self.parent_select  = nsga3.parent_selection   # global parent picker
        logging.info(
            "NSGA-III params -> n_obj=%s pop_size=%s n_partitions=%s random_mutation=%s llm_crossover=%s (ignored in NSGA-III: n_neighbors=%s nr=%s)",
            self.n_obj, self.popsize, self.n_partitions,
            self.random_mutation, self.llm_crossover, self.n_neighbors, self.nr
        )
        # ─────────────────────────────────────────────────────────────────────
    ...
    def crossover(self, population: List[Individual], generation: int, mut_start_offset: int = 0) -> List[Individual]:
        """
        Produce one child per selection round via crossover.

        For each index *i*:
          1. ``parent_selection`` picks (parent_a, parent_b) from the
             whole population (NSGA-III variant).
          2. crossover computes which positions differ and which alleles
             the child inherits from parent_b.
        ...
        Returns
        -------
        List[Individual]  – one child per index (same length as population)
        """
        ...
        parent_pairs = self.parent_select(
            population=population,
            sub_problem_idx=0,
            n_parent_pairs=len(population),
        )

        for i in range(len(population)):
            parent_a, parent_b = parent_pairs[i]
    ...
    def update_pareto_front(self, pareto_front: List[Individual], population: List[Individual]) -> List[Individual]:
        ...
        # Keep individuals not dominated by any other solution (minimization space).
    ...
    def run(self, checkpoint: bool = False, freq: int = 2):
        ...
        for generation in range(ngen_start, self.ngenerations):
            ...
            # Step 1: Parent selection + crossover (one child per index)
            ...
            # Step 4: NSGA-III survival update
            logging.info(f"[Gen {generation}] Selection (NSGA-III survival)...")
