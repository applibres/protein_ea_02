...
for (( K=INIT; K<=END; K+=1 )); do 
  ./protmut_run.sh test06 sea pdbfile=9Q1V_prepared_clean_relaxed.pdb,partners=A_B,ligand_chain=B gen=20,popsize=25,mutp=0.3 fitness_idsx=7 checkpoint=False checks=2 mobj=True randomseed=$K
  if [ $? -ne 0 ]; then
